    ...
    if response.status_code == 200:
        ...
        for category in categories:
            ...
            for process in list_of_processes['data']:
                if process["tipoInfracao"] == category:   # "tipoInfracao" -> Chave não é usada para fazer consulta e sim criar objetos
                    list_municipio.append(process["municipio"])   #append quer dizer adicione no final
            ...
            row = {'municipio': list_municipio, 'nomeRazaoSocial': list_nomeRazaoSocial, 'valorAuto': list_valorAuto,'dataAuto': list_dataAuto, 'situcao': list_situcaoDebito}
            df = pd.DataFrame(row, columns=['municipio', 'nomeRazaoSocial', 'valorAuto', 'dataAuto', 'situcaoDebito'])
            df.to_csv('%s.csv' % category)
            print ("Multas relacionados a %s foram salvas na tabel!" % category)
